...
internal sealed class RecordingMaintenanceRunner : ISettlementMaintenanceRunner
{
    ...
    public Task<CommerceFulfillmentReport> ProcessDueFulfillmentsAsync(
        CancellationToken cancellationToken)
    {
        Calls.Add(nameof(ProcessDueFulfillmentsAsync));

        return Task.FromResult(new CommerceFulfillmentReport(2, 1, 1));
    }

    public Task<AtmInstallationRecoveryReport> ScanAtmInstallationsAsync(
        CancellationToken cancellationToken)
    {
        Calls.Add(nameof(ScanAtmInstallationsAsync));

        return Task.FromResult(new AtmInstallationRecoveryReport(1, 1, 0));
    }

    public Task<ScheduledPaymentMaintenanceReport> ProcessDueScheduledPaymentsAsync(
        CancellationToken cancellationToken)
    {
        Calls.Add(nameof(ProcessDueScheduledPaymentsAsync));

        return Task.FromResult(new ScheduledPaymentMaintenanceReport(2, 1, 2));
    }
...
public sealed class SettlementMaintenanceWorkerTests
{
    ...
    public async Task OneTickRunsQueuedSettlementBeforeClearingCycles()
    {
        ...
        CollectionAssert.AreEqual(
            new[]
            {
                ...
                "ProcessDueFulfillmentsAsync",
                "ScanAtmInstallationsAsync",
                "ProcessDueScheduledPaymentsAsync",
    ...
    public async Task OneTickReportsTheCombinedCounts()
    {
        ...
        Assert.AreEqual(19, diagnostics.LastExamined);
        Assert.AreEqual(15, diagnostics.LastSettled);
    }
